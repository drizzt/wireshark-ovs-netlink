 ...
 * Routines for netlink-ovs_meter dissection
    ...
    int nla_type, int offset, int len)
{
    ...
    case WS_OVS_BAND_ATTR_STATS:
        ...
        if (len == 16) {
            pi = proto_tree_add_subtree_format(tree, tvb, offset, len, 0, &pi, "Band Statistics");
    ...
    int nla_type, int offset, int len)
{
    ...
    case WS_OVS_METER_ATTR_KBPS:
        /* Flag attribute -- rate in kbps when present */
        return 1;

    case WS_OVS_METER_ATTR_STATS:
        /* struct ovs_flow_stats: n_packets(u64)+n_bytes(u64)=16 */
        if (len == 16) {
            pi = proto_tree_add_subtree_format(tree, tvb, offset, len, 0, &pi, "Meter Statistics");
            ptree = proto_item_add_subtree(pi,
                ett_ovs_meter_stats);
            proto_tree_add_item(ptree,
    ...
    case WS_OVS_METER_ATTR_BANDS:
        return ovs_dissect_netlink_attributes_array(tvb,
    ...
    proto_tree *tree, void *data)
{
...

    {
        struct netlink_ovs_meter_info info;
        info.pinfo = pinfo;
        ovs_dissect_netlink_attributes_to_end(tvb, hf_ovs_meter_attr,
