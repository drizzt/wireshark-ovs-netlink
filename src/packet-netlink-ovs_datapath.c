 ...
 * Routines for netlink-ovs_datapath dissection
...
static int
dissect_ovs_dp_attrs(tvbuff_t *tvb, void *data _U_,
    ...
    int nla_type, int offset, int len)
{
    ...
    case WS_OVS_DP_ATTR_NAME:
        {
            struct netlink_ovs_dp_info *info =
                (struct netlink_ovs_dp_info *) data;
    ...
    case WS_OVS_DP_ATTR_STATS:
        ...
        if (len == 32) {
            int off = offset;
            pi = proto_tree_add_subtree_format(tree, tvb, offset, len, 0, &pi, "Datapath Statistics");
    ...
    case WS_OVS_DP_ATTR_MEGAFLOW_STATS:
        ...
        if (len >= 16) {
            int off = offset;
            pi = proto_tree_add_subtree_format(tree, tvb, offset, len, 0, &pi, "Megaflow Statistics");
    ...
    proto_tree *tree, void *data)
{
...

    {
        struct netlink_ovs_dp_info info;
        info.pinfo = pinfo;
        ovs_dissect_netlink_attributes_to_end(tvb, hf_ovs_dp_attr,
