 ...
 * Routines for netlink-ovs_packet dissection
...
static int hf_ovs_packet_key;
static int hf_ovs_packet_actions;
static int hf_ovs_packet_userdata;
static int hf_ovs_packet_egress_tun_key;
    ...
    int nla_type, int offset, int len)
{
    ...
    case WS_OVS_PACKET_ATTR_KEY:
        proto_tree_add_item(tree, hf_ovs_packet_key, tvb,
            offset, len, ENC_NA);
        return 1;

    case WS_OVS_PACKET_ATTR_ACTIONS:
        proto_tree_add_item(tree, hf_ovs_packet_actions, tvb,
            offset, len, ENC_NA);
        return 1;
    ...
    case WS_OVS_PACKET_ATTR_EGRESS_TUN_KEY:
        proto_tree_add_item(tree, hf_ovs_packet_egress_tun_key, tvb,
            offset, len, ENC_NA);
        return 1;

    case WS_OVS_PACKET_ATTR_PROBE:
        /* Flag attribute -- presence alone is meaningful */
        return 1;
    ...
    proto_tree *tree, void *data)
{
    ...
    ovs_dissect_netlink_attributes_to_end(tvb, hf_ovs_packet_attr,
        ett_ovs_packet_attrs, NULL, genl_info->nl_data, nlmsg_tree,
...
proto_register_netlink_ovs_packet(void)
{
    static hf_register_info hf[] = {
        ...
        { &hf_ovs_packet_actions,
            { "Actions", "ovs_packet.actions",
              FT_BYTES, BASE_NONE, NULL, 0x00, NULL, HFILL }
        },
        ...
        { &hf_ovs_packet_egress_tun_key,
            { "Egress tunnel key",
              "ovs_packet.egress_tun_key",
              FT_BYTES, BASE_NONE, NULL, 0x00, NULL, HFILL }
        },
        { &hf_ovs_packet_mru,
            { "MRU", "ovs_packet.mru",
